...
<title>Shell Finder v1.0</title>


<div class="titlee">
<center>
<h1> Shell Finder v1.0 </h1>
</center>
</div> 
<center>

<div class="walkerlogo">
<img src="https://res.cloudinary.com/teepublic/image/private/s--2hhdeUb0--/t_Preview/b_rgb:191919,c_limit,f_auto,h_313,q_90,w_313/v1521364964/production/designs/2486221_0" width="130" height="130">
...
<style type="text/css">
body {
 background: url(https://cdn.pixabay.com/photo/2019/12/24/15/38/hacker-4717018_1280.png) no-repeat center center fixed; 
...
.walkerlogo{
	...
	height:130px;
	border-radius:2px;
	border-color: #c41d1d solid 2px;
border: 8px solid #d32323;
font-family:courier new;
font-weight:bold;
}
.walkerlogo:hover{
...
<div class="red">
<div class="tube">
<form action="" method="post" name="walk3rform">
<br /><center><input placeholder="Use http://" class="inpute" type="text" name="walk33r" value="' . $_POST['walk33r'] . '" /></center>
<br><br>
<center><input type="submit" class="submite" name="walk3rsubmit" value="Submit" align="center" /></center>
</form>
<br />
</div> <!-- /tube -->
</div> <!-- /red -->
<br />
<div class="a.link">
<center> <h1><a href="https://mrlinkerrorsystem.blogspot.com">Coded By Pace Usa</a></h1></center>
<br/>
<div class="locked">
<div class="tube" id="rightcol">
<font size="3">Checked Shells: <span id="verified">0</span> / <span id="total">0</span></font><br />
<font size="3">Shells Found : </font><br />
</div> <!-- /tube -->
</div> <!-- /green -->
...
function check($x, $front = 0) {
    ...
    if ($_POST['walk33rstringgs'] == "") if ($data == $false) return 0;
    if ($_POST['walk33rstringgs'] != "") if (strpos($data, $_POST['walk33rstringgs'])) return 0;
    $walk33rstringgs = "404 not found";
    return 1;
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
template();
if (!isset($_POST['walk33r'])) die;
if ($_POST['walk33r'] == '') die;
$site = $_POST['walk33r'];
if ($site[strlen($site) - 1] != "/") $site.= "/";
if ($_POST['walk33rstringgs'] == "") $false = @file_get_contents($site . "d65897f5380a21a42db94b3927b823d56ee1099a-this_can-t_exist.html");
...
foreach ($pathes as $path) {
    $walkerverif++;
    show($walkerverif, 0, 0, 'verified', 1);
    if (check($path) == 0) show(1, 0, 'logbox', 0);
    else {
...
foreach ($frontpathes as $frontpath) {
    $walkerverif++;
    show($walkerverif, 0, 0, 'verified', 1);
    if (check($frontpath, 1) == 0) show(1, 0, 'logbox', 0);
    else {
